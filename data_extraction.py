...
class DataExtractor:
    def __init__(self, db_connector):
        # Store the DatabaseConnector instance for later use
    ...
    def list_number_of_stores(self, number_stores_url, headers):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error retrieving number of stores: {response.status_code}, {response.text}"
            )
            return None
        ...
        for store_number in range(1, number_of_stores):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Error retrieving data for store {store_number}: {response.status_code}"
                    )
